...
def profile_dpla(base_dpla_filename):
    ...
    # output base dpla-level data
    get_base_dpla_data(FILE_path + base_dpla_filename)

    # for each provider, retrieve and process the data
    CCD_OBJ_provider_parent = get_provider_data()

    # pprint(CCD_OBJ_provider_parent)

    # output provider data separately
    output_provider_data_seperately(CCD_OBJ_provider_parent)

    # sys.exit(0)

    """
    get collection data by collection id
    parameters: collection_id, collection_only, filename
    """
    get_collection_data_by_collId('5199919781c678cdee2566c339d4ca44', True, 'test.json')

def get_base_dpla_data(base_dpla_filename):
    # base object to store the base dpla-level data
    CCD_OBJ_dpla_parent = {}

    # set admin data for CCD_OBJ_dpla_parent
    # "adminDetails": {"analysisDate": "", "analysisTime": ""}
    CCD_OBJ_adminDetails = get_admin_data()
    ...
    response_collection_id_facets = retrieve_facets_by_collection_id()

    CCD_OBJ_dplaData[CCD_PROP_LBL_itemsInCollections] = \
      response_collection_id_facets[DPLA_PROP_collectionId][DPLA_API_PROP_total]
    ...
    # write_file(base_dpla_filename, CCD_OBJ_dpla_parent)
...
def output_provider_data_seperately(providers):
    # save the provider data to file
    for provider in providers:
        CCD_OBJ_provider = {}
        CCD_OBJ_provider[CCD_PROP_LBL_itemCount] = provider[CCD_PROP_LBL_itemCount]
        CCD_OBJ_provider[CCD_PROP_LBL_dataProviderCount] = provider[CCD_PROP_LBL_dataProviderCount]

        provider_filename = provider[DPLA_PROP_providerName].replace(' ','-') + FILE_SUFFIX_provider_data
        provider_data_file = open(FILE_path + provider_filename, 'w')
        provider_data_file.write(json.dumps(CCD_OBJ_provider))
...
def get_collection_data_by_collId(collection_id, collection_only, filename):
    id_list = []
    id_duplicated = []
    ...
    dest_file.write(json.dumps(collection))
    dest_file.close()
